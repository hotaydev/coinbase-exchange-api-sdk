import {
  IDepositFromCoinbaseAccount,
  IDepositFromPaymentMethod,
  IDepositResponse,
  IFeeEstimate,
  IFeeEstimateRequestQueryParams,
  IPaymentMethods,
  ITransfer,
  ITransferQueryParams,
  ITravelInformationForTransfer,
  IWithdrawResponse,
  IWithdrawToCoinbaseAccount,
  IWithdrawToCryptoAddress,
  IWithdrawToPaymentMethod,
...
export default class TransfersEndpoint {
   ...
   */
  public async getFeeEstimateForCryptoWithdrawal(
    query: IFeeEstimateRequestQueryParams,
   ...
   */
  public async depositFromCoinbaseAccount(
    body: IDepositFromCoinbaseAccount,
   ...
   * @returns Promise<IDepositResponse>
   */
  public async depositFromPaymentMethod(
    body: IDepositFromPaymentMethod,
  ): Promise<IDepositResponse> {
    return (await this.instance.request({
   ...
   */
  public async submitTravelInformationForTransfer(
    transferId: string,
    body: ITravelInformationForTransfer,
  ): Promise<IGeneralResponse> {
    return (await this.instance.request({
      path: Endpoints.createTransferTravelInformation.replace(
        ":id",
        transferId,
      ),
      method: "POST",
   ...
   */
  public async withdrawToCoinbaseAccount(
    body: IWithdrawToCoinbaseAccount,
   ...
   * @returns Promise<IWithdrawResponse>
   */
  public async withdrawToCryptoAddress(
    body: IWithdrawToCryptoAddress,
  ): Promise<IWithdrawResponse> {
    return (await this.instance.request({
   ...
   * @returns Promise<IWithdrawResponse>
   */
  public async withdrawToPaymentMethod(
    body: IWithdrawToPaymentMethod,
  ): Promise<IWithdrawResponse> {
    return (await this.instance.request({
