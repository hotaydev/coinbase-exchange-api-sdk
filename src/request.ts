...
export default class CoinbaseTradeRequest {
    ...
    private getSign(timestamp: number, method: string, path: string, body: Object): string {
    ...
    public initialize({
        key, passphrase, secret, environment
    ...
    public async request(method: string, path: string, body: Object): Promise<unknown> {
        const timestamp = this.getTimestamp();
        const sign = this.getSign(timestamp, method, this.startWithSlash(path), body);
